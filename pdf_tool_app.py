...
# 記得將下方網址替換成你在該 GitHub 專案中字型檔案的 Raw 網址
github_font_url = "https://github.com/rin047/HS-PDF-binder/raw/refs/heads/main/NotoSansTC-VariableFont_wght.ttf"
    ...
    if st.button("🚀 確認刪除所選頁面、生成考卷目錄並組合新 PDF", type="primary"):
        if not selected_page_ids:
            ...
            try:
                ...
                # 註冊中文字型（結合 GitHub 自動下載與本機備用機制）
                font_name = "Helvetica"
                if reportlab_available:
                    font_registered = False
                    try:
                        # 確保點擊按鈕時若檔案不存在會再次嘗試下載
                        if not os.path.exists(font_path):
                            res = requests.get(github_font_url, timeout=10)
                            if res.status_code == 200:
                                with open(font_path, "wb") as f:
                ...
                st.success("🎉 PDF 處理完畢，已成功從 GitHub 抓取字型並產生成品！")
                st.download_button(
