...
if uploaded_files:
    ...
    for f_idx, file in enumerate(uploaded_files):
        ...
        if len(doc) > 0:
            ...
            if lines:
                # 通常考卷第一行或前幾行就是標題，挑選最長或含有「學年度」的行
        ...
        for p_idx, page in enumerate(doc):
            ...
            all_pages_info.append({
                ...
                "file_name": file.name,
                "exam_title": detected_title,  # 記錄抓取到的標題
    ...
    if "selected_states" not in st.session_state or st.session_state.get("last_files_len") != len(uploaded_files):
        st.session_state["selected_states"] = {item["id"]: True for item in all_pages_info}
            ...
            if i + j < len(all_pages_info):
                ...
                with row_cols[j]:
                    ...
                    is_checked = st.checkbox(
                        "保留此頁", 
                        value=st.session_state["selected_states"].get(item["id"], True),
                        key=f"chk_{item['id']}"
                    )
                    st.session_state["selected_states"][item["id"]] = is_checked
                    if is_checked:
    ...
    with col2:
        add_title_flag = st.checkbox("在第一張內容頁頂部加上大標題", value=True)
    with col3:
        ...
        if not selected_page_ids:
            ...
            try:
                ...
                for idx, info in enumerate(selected_items):
                    ...
                    actual_page_num = idx + 1 + toc_page_count  # 內容頁在總 PDF 中的頁碼
                ...
                if reportlab_available:
                    try:
                        ...
                        if os.path.exists(font_path):
                            pdfmetrics.registerFont(TTFont("CustomMSJH", font_path, subfontIndex=0))
                            font_name = "CustomMSJH"
                        else:
                            font_path_2 = "C:/Windows/Fonts/simhei.ttf"
                            if os.path.exists(font_path_2):
                                pdfmetrics.registerFont(TTFont("CustomSimHei", font_path_2))
                                font_name = "CustomSimHei"
                    except Exception:
                        pass
