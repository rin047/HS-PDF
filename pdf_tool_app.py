        ...
        if not selected_page_ids:
            ...
            try:
                ...
                if reportlab_available:
                    try:
                        ...
                        if os.path.exists(font_path):
                            # 設定 weight=700 讓字體變粗
                            pdfmetrics.registerFont(TTFont("CustomBold", font_path, weight=700))
                            font_name = "CustomBold"
                        else:
                            font_path_win = "C:/Windows/Fonts/msjhbd.ttc"  # Windows 微軟正黑體粗體
                            if os.path.exists(font_path_win):
                                pdfmetrics.registerFont(TTFont("CustomBold", font_path_win, subfontIndex=0))
                                font_name = "CustomBold"
                    except Exception:
                        pass
                ...
                if add_toc_flag:
                    ...
                    if add_footer_flag:
                        toc_can.setDash([])
                        toc_can.setFont(font_name, 14)  # 目錄頁頁碼加大並加粗
                        toc_can.setFillColorRGB(0.1, 0.1, 0.1)
                ...
                st.success("🎉 PDF 處理完畢，已成功產生粗體目錄與大號加粗頁碼！")
                st.download_button(
